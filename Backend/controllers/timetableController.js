...
export const addSubject = async (req, res) => {
  try {
    const { day, startTime, endTime, room, subject, type } = req.body;
    const userId = req.user._id;
...
export const deleteSubject = async (req, res) => {
  try {
    ...
    const subjectIndex = timetable.schedule.findIndex(subject => subject._id == subjectId);

    if (subjectIndex > -1) {
      timetable.schedule.splice(subjectIndex, 1);
      await timetable.save();
