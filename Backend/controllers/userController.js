...
const usersController = {
    ...
    updateUserProfile: asyncHandler(async (req, res) => {
        const { email, username, mobileNo, semester, DOB, course, regnum, auraPoints, badges } = req.body;

        const updatedUser = await User.findByIdAndUpdate(
            req.user,
            { email, username, mobileNo, semester, DOB, course, regnum, auraPoints, badges },
            { new: true }
        );
    ...
    dailyCheckIn: asyncHandler(async (req, res) => {
        ...
        try {
            ...
            const lastCheckDate = user.lastCheckIn ? new Date(user.lastCheckIn).setHours(0, 0, 0, 0) : null;
            ...
            res.status(200).json({ message: "Daily check-in successful!", auraPoints: user.auraPoints, badges: user.badges });
        } catch (error) {
    ...
    updateAuraPoints: asyncHandler(async (req, res) => {
        ...
        res.status(200).json({ message: "Points updated successfully!", auraPoints: updatedUser.auraPoints, badges: updatedUser.badges });
    }),
    updateBadges: asyncHandler(async (req, res) => {
        ...
        await user.save();
        res.status(200).json({ message: "Badges updated successfully", badges: user.badges });
    })
};
